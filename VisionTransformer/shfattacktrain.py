...
class MLP_CE(nn.Module):
    def __init__(self, selected_posteriors=2):
        ...
        self.input_dim = 2*(selected_posteriors + 1)
...
class attackTraining():
    def __init__(self, opt, targetTrainloader, targetTestloader, shadowTrainloader, shadowTestloader, target_model, shadow_model, attack_model, device):
        self.device = device
        self.opt = opt
        self.jigsaw = JigsawPuzzleMaskedRegion(img_size=32,
                                              patch_size=8
                                              )
    ...
    def get_data(self, dataloader, model_type="target", member_type=1):
        ...
        with torch.no_grad():
            for batch_idx, (inputs, targets) in tqdm(enumerate(dataloader), desc="process"):
                ...
                inputs_mask, _ = self.jigsaw(inputs)
                if model_type == "shadow":
                    outputs = self.shadow_model(inputs)
                    outputs_mask = self.shadow_model(inputs_mask)
                ...
                _, predicted_mask = outputs_mask.max(1)
                total += targets.size(0)
                correct += predicted.eq(targets).sum().item()

                if torch.cuda.is_available():
                    outputs = outputs.cpu()
                    outputs_mask = outputs_mask.cpu()

                outputs = outputs.numpy().tolist()
                outputs_mask = outputs_mask.numpy().tolist()
                s = predicted.eq(targets).cpu().numpy().tolist()
                s_mask = predicted_mask.eq(targets).cpu().numpy().tolist()


                for i in range(len(outputs)):
                    outputs[i].sort(reverse=True)
                    outputs_mask[i].sort(reverse=True)
                    data.append(
                        [outputs[i][:self.selected_posterior] + [float(s[i])]
                         + outputs_mask[i][:self.selected_posterior] + [float(s_mask[i])]
                         ,member_type])
