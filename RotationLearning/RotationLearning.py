...
def train_model(model, criterion, optimizer, scheduler, device, checkpoint_path, f, verbIter, num_epochs=25):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            epoch_loss = running_loss / n_samples
            if phase=='train':
                acc_npy[0][epoch] = top_1_acc
            else:
                acc_npy[1][epoch] = top_1_acc
            ...
            if phase == 'val' and top_1_acc > best_acc:
                best_acc = top_1_acc
                best_model_wts = copy.deepcopy(model.state_dict())
    np.save('./res/'+str(now)+'.npy',acc_npy)
...
parser.add_argument('--dataroot', default='../data/cinic-10', help='path to dataset')
...
parser.add_argument('--verbIter', type=int, default=10, help='number of batches for interval printing')
