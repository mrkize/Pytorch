...
parser.add_argument('--dataroot', default='../data/cifar-100_rotate', help='path to dataset')
...
image_datasets = {x: RotationDataset(x, opt.dataroot, data_pre_transforms[x], data_post_transforms[x]) for x in
                  ['train', 'val']}
assert image_datasets
dataloaders = {
    x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchSize, pin_memory=True, shuffle=True,
...
model_ft = train_model(model_ft, criterion, optimizer_conv, exp_lr_scheduler, device, opt.outf, f, opt.verbIter,
