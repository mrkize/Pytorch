...
def experience_cifar10(config, path, args):

    print(f"Start data: {args.data} , mode {args.model}")
    # get device
    print(config)
    ...
    if args.target_path == '':
        model_target, train_loader_target, test_loader_target = get_model(args.model, args.data, istarget=True, not_train=args.not_train, config=config ,model_path=mode_path, res_path=path)

        model_shadow, train_loader_shadow, test_loader_shadow = get_model(args.model, args.data, istarget=False, not_train=args.not_train, config=config ,model_path=mode_path, res_path=path)
    else:
        model_target = torch.load(args.target_path+'target.pth')
        model_shadow = torch.load(args.target_path+'shadow.pth')
    ...
    if args.data == "cifar10":
        ...
        data_train_distill = custum_CINIC10('distill', train=True, config = config).dataset
        data_test_distill = custum_CINIC10('distill', train=True, config = config).dataset
        args.num_class = 10
    ...
    if not args.not_distill:
        ...
        distill_model(model_target, train_loader_distill, len(data_train_distill), config, True, args)
    ...
    print("The accuracy of model is: {}".format(ret_para_attack[6][config.learning.epochs-1]))
